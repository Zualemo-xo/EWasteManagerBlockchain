...
import CenterContract from "../../contracts/CenterContract.json";
...
class Login extends Component {
    state = { web3: null, accounts: null, CenterContract: null, loading: false, isaddr: "" };
    ...
    componentDidMount = async () => {
        try {
            ...
            const deployedNetwork = CenterContract.networks[networkId];
            const instance = new web3.eth.Contract(
                CenterContract.abi,
                deployedNetwork && deployedNetwork.address,
            );

            this.setState({ web3, accounts, CenterContract: instance });
        } catch (error) {
    ...
    handlesignIn = async () => {
        ...
        const contract = this.state.CenterContract;
        ...
        const response = await contract.methods.getCenterID(metamaskAddr).call();
    ...
    handlesignUp = async (event) => {
        ...
        await contract.methods.registerUser(metamaskAddr, name, email, phone, Haddr, area_code).send({ from: metamaskAddr });
    ...
    render() {
        if (localStorage.getItem('centerID')) {
            return <Navigate to="../../public/profile" />;
        }
        if (!this.state.web3) {
        ...
        return (
            ...
            <div className="App">
                <h1 className="user-h1-font">Collection Center Login</h1>
